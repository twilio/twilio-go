/*
 * This code was generated by
 * ___ _ _ _ _ _    _ ____    ____ ____ _    ____ ____ _  _ ____ ____ ____ ___ __   __
 *  |  | | | | |    | |  | __ |  | |__| | __ | __ |___ |\ | |___ |__/ |__|  | |  | |__/
 *  |  |_|_| | |___ | |__|    |__| |  | |    |__] |___ | \| |___ |  \ |  |  | |__| |  \
 *
 * Twilio - Insights
 * The Twilio Insights v3 API provides semantic query access to Conversations analytics data. Resources include synchronous Query, Metadata, and asynchronous QueryJobs for long-running queries.
 *
 * NOTE: This class is auto generated by OpenAPI Generator.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

package openapi

// LongRunningOperationStatus The valid status values for long-running operations.
type LongRunningOperationStatus string

// List of LongRunningOperationStatus
const (
	LONGRUNNINGOPERATIONSTATUS_PENDING   LongRunningOperationStatus = "PENDING"
	LONGRUNNINGOPERATIONSTATUS_RUNNING   LongRunningOperationStatus = "RUNNING"
	LONGRUNNINGOPERATIONSTATUS_CANCELLED LongRunningOperationStatus = "CANCELLED"
	LONGRUNNINGOPERATIONSTATUS_COMPLETED LongRunningOperationStatus = "COMPLETED"
	LONGRUNNINGOPERATIONSTATUS_FAILED    LongRunningOperationStatus = "FAILED"
)
