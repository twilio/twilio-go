...
func (c *ApiService) RoomsParticipantsPage(RoomSid string, params *ListRoomParticipantParams, pageToken string, pageNumber string) (*ListRoomParticipantResponse, error) {
	path := "/v1/Rooms/{RoomSid}/Participants"
	path = strings.Replace(path, "{"+"RoomSid"+"}", RoomSid, -1)
...
//Lists RoomsParticipants records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) RoomsParticipantsList(RoomSid string, params *ListRoomParticipantParams, limit int) ([]ListRoomParticipantResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListRoomParticipant(RoomSid, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListRoomParticipantResponse, len(resp))
...
//Streams RoomsParticipants records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) RoomsParticipantsStream(RoomSid string, params *ListRoomParticipantParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListRoomParticipant(RoomSid, params)
	if err != nil {
		return nil, err
