...
func (c *ApiService) ListInteractionChannelParticipant(InteractionSid string, ChannelSid string, params *ListInteractionChannelParticipantParams) ([]FlexV1InteractionChannelParticipant, error) {
	response, err := c.StreamInteractionChannelParticipant(InteractionSid, ChannelSid, params)
	if err != nil {
		return nil, err
	}
...
func (c *ApiService) StreamInteractionChannelParticipant(InteractionSid string, ChannelSid string, params *ListInteractionChannelParticipantParams) (chan FlexV1InteractionChannelParticipant, error) {
	if params == nil {
	...
	channel := make(chan FlexV1InteractionChannelParticipant, 1)
	...
	go func() {
		for response != nil {
			...
			if record, err = client.GetNext(c.baseURL, response, c.getNextListInteractionChannelParticipantResponse); record == nil || err != nil {
				close(channel)
				return
			}
