...
func (c *ApiService) ServicesMapsPermissionsPage(ServiceSid string, MapSid string, params *ListSyncMapPermissionParams, pageToken string, pageNumber string) (*ListSyncMapPermissionResponse, error) {
	path := "/v1/Services/{ServiceSid}/Maps/{MapSid}/Permissions"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
	path = strings.Replace(path, "{"+"MapSid"+"}", MapSid, -1)
...
//Lists ServicesMapsPermissions records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ServicesMapsPermissionsList(ServiceSid string, MapSid string, params *ListSyncMapPermissionParams, limit int) ([]ListSyncMapPermissionResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSyncMapPermission(ServiceSid, MapSid, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListSyncMapPermissionResponse, len(resp))
...
//Streams ServicesMapsPermissions records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) ServicesMapsPermissionsStream(ServiceSid string, MapSid string, params *ListSyncMapPermissionParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSyncMapPermission(ServiceSid, MapSid, params)
	if err != nil {
		return nil, err
