...
func (c *ApiService) ServicesMapsPage(ServiceSid string, params *ListSyncMapParams, pageToken string, pageNumber string) (*ListSyncMapResponse, error) {
	path := "/v1/Services/{ServiceSid}/Maps"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
...
//Lists ServicesMaps records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ServicesMapsList(ServiceSid string, params *ListSyncMapParams, limit int) ([]ListSyncMapResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSyncMap(ServiceSid, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListSyncMapResponse, len(resp))
...
//Streams ServicesMaps records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) ServicesMapsStream(ServiceSid string, params *ListSyncMapParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSyncMap(ServiceSid, params)
	if err != nil {
		return nil, err
