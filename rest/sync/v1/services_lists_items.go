...
func (c *ApiService) SyncListItemPage(ServiceSid string, ListSid string, params *ListSyncListItemParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Services/{ServiceSid}/Lists/{ListSid}/Items"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
	path = strings.Replace(path, "{"+"ListSid"+"}", ListSid, -1)
...
func (c *ApiService) SyncListItemStream(ServiceSid string, ListSid string, params *ListSyncListItemParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SyncListItemPage(ServiceSid, ListSid, params, "", "", fmt.Sprint(limits.PageSize))
...
func (c *ApiService) SyncListItemList(ServiceSid string, ListSid string, params *ListSyncListItemParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SyncListItemPage(ServiceSid, ListSid, params, "", "", fmt.Sprint(limits.PageSize))
