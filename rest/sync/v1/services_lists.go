...
func (c *ApiService) SyncListPage(ServiceSid string, params *ListSyncListParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Services/{ServiceSid}/Lists"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
...
func (c *ApiService) SyncListStream(ServiceSid string, params *ListSyncListParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SyncListPage(ServiceSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists SyncList records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) SyncListList(ServiceSid string, params *ListSyncListParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SyncListPage(ServiceSid, params, "", "", fmt.Sprint(limits.PageSize))
