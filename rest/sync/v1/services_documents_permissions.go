...
func (c *ApiService) DocumentPermissionPage(ServiceSid string, DocumentSid string, params *ListDocumentPermissionParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Services/{ServiceSid}/Documents/{DocumentSid}/Permissions"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
	path = strings.Replace(path, "{"+"DocumentSid"+"}", DocumentSid, -1)
...
func (c *ApiService) DocumentPermissionStream(ServiceSid string, DocumentSid string, params *ListDocumentPermissionParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.DocumentPermissionPage(ServiceSid, DocumentSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists DocumentPermission records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) DocumentPermissionList(ServiceSid string, DocumentSid string, params *ListDocumentPermissionParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.DocumentPermissionPage(ServiceSid, DocumentSid, params, "", "", fmt.Sprint(limits.PageSize))
