...
func (c *ApiService) SamplePage(AssistantSid string, TaskSid string, params *ListSampleParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Assistants/{AssistantSid}/Tasks/{TaskSid}/Samples"
	path = strings.Replace(path, "{"+"AssistantSid"+"}", AssistantSid, -1)
	path = strings.Replace(path, "{"+"TaskSid"+"}", TaskSid, -1)
...
func (c *ApiService) SampleStream(AssistantSid string, TaskSid string, params *ListSampleParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SamplePage(AssistantSid, TaskSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists Sample records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) SampleList(AssistantSid string, TaskSid string, params *ListSampleParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SamplePage(AssistantSid, TaskSid, params, "", "", fmt.Sprint(limits.PageSize))
