/*
 * This code was generated by
 * ___ _ _ _ _ _    _ ____    ____ ____ _    ____ ____ _  _ ____ ____ ____ ___ __   __
 *  |  | | | | |    | |  | __ |  | |__| | __ | __ |___ |\ | |___ |__/ |__|  | |  | |__/
 *  |  |_|_| | |___ | |__|    |__| |  | |    |__] |___ | \| |___ |  \ |  |  | |__| |  \
 *
 * Conversation Orchestrator
 * Manage configurations, conversations, participants, and communications. Create configurations to define capture rules and channel settings, then use conversations to group related communications.
 *
 * NOTE: This class is auto generated by OpenAPI Generator.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

package openapi

import (
	"time"
)

// ListConversationByAccountResponseConversations struct for ListConversationByAccountResponseConversations
type ListConversationByAccountResponseConversations struct {
	// Conversation ID.
	Id string `json:"id"`
	// Account ID.
	AccountId string `json:"accountId"`
	// Configuration ID.
	ConfigurationId string `json:"configurationId"`
	// Conversation status.
	Status string `json:"status,omitempty"`
	// Conversation name.
	Name *string `json:"name,omitempty"`
	// Timestamp when this Conversation was created.
	CreatedAt time.Time `json:"createdAt,omitempty"`
	// Timestamp when this Conversation was last updated.
	UpdatedAt     time.Time                                                   `json:"updatedAt,omitempty"`
	Configuration ListConversationByAccountResponseConversationsConfiguration `json:"configuration,omitempty"`
	// Participants in this Conversation.
	Participants []ConversationsV2Participant `json:"participants,omitempty"`
}
