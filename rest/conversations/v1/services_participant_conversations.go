...
func (c *ApiService) ListServiceParticipantConversation(ChatServiceSid string, params *ListServiceParticipantConversationParams) ([]ConversationsV1ServiceParticipantConversation, error) {
	...
	for response != nil {
		...
		if record, err = client.GetNext(response, &curRecord, params.Limit, c.getNextListServiceParticipantConversationResponse); record == nil || err != nil {
			return records, err
		}
...
func (c *ApiService) getNextListServiceParticipantConversationResponse(nextPageUri string) (interface{}, error) {
	if nextPageUri == "" {
		return nil, nil
	}
