...
func (c *ApiService) ServiceConversationPage(ChatServiceSid string, params *ListServiceConversationParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Services/{ChatServiceSid}/Conversations"
	path = strings.Replace(path, "{"+"ChatServiceSid"+"}", ChatServiceSid, -1)
...
func (c *ApiService) ServiceConversationStream(ChatServiceSid string, params *ListServiceConversationParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ServiceConversationPage(ChatServiceSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists ServiceConversation records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ServiceConversationList(ChatServiceSid string, params *ListServiceConversationParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ServiceConversationPage(ChatServiceSid, params, "", "", fmt.Sprint(limits.PageSize))
