...
func (c *ApiService) ServiceUserPage(ChatServiceSid string, params *ListServiceUserParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Services/{ChatServiceSid}/Users"
	path = strings.Replace(path, "{"+"ChatServiceSid"+"}", ChatServiceSid, -1)
...
func (c *ApiService) ServiceUserStream(ChatServiceSid string, params *ListServiceUserParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ServiceUserPage(ChatServiceSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists ServiceUser records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ServiceUserList(ChatServiceSid string, params *ListServiceUserParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ServiceUserPage(ChatServiceSid, params, "", "", fmt.Sprint(limits.PageSize))
