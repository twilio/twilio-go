...
func (c *ApiService) ListParticipantConversation(params *ListParticipantConversationParams) ([]ConversationsV1ParticipantConversation, error) {
	...
	for response != nil {
		...
		if record, err = client.GetNext(response, &curRecord, params.Limit, c.getNextListParticipantConversationResponse); record == nil || err != nil {
			return records, err
		}
...
func (c *ApiService) getNextListParticipantConversationResponse(nextPageUri string) (interface{}, error) {
	if nextPageUri == "" {
		return nil, nil
	}
