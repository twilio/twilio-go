...
func (c *ApiService) PageParticipantConversation(params *ListParticipantConversationParams, pageToken, pageNumber string) (*ListParticipantConversationResponse, error) {
	return c.PageParticipantConversationWithCtx(context.TODO(), params, pageToken, pageNumber)
}

// Retrieve a single page of ParticipantConversation records from the API. Request is executed immediately.
func (c *ApiService) PageParticipantConversationWithCtx(ctx context.Context, params *ListParticipantConversationParams, pageToken, pageNumber string) (*ListParticipantConversationResponse, error) {
...
func (c *ApiService) ListParticipantConversation(params *ListParticipantConversationParams) ([]ConversationsV1ParticipantConversation, error) {
	return c.ListParticipantConversationWithCtx(context.TODO(), params)
}

// Lists ParticipantConversation records from the API as a list. Unlike stream, this operation is eager and loads 'limit' records into memory before returning.
func (c *ApiService) ListParticipantConversationWithCtx(ctx context.Context, params *ListParticipantConversationParams) ([]ConversationsV1ParticipantConversation, error) {
	response, errors := c.StreamParticipantConversationWithCtx(ctx, params)
...
func (c *ApiService) StreamParticipantConversation(params *ListParticipantConversationParams) (chan ConversationsV1ParticipantConversation, chan error) {
	return c.StreamParticipantConversationWithCtx(context.TODO(), params)
}

// Streams ParticipantConversation records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) StreamParticipantConversationWithCtx(ctx context.Context, params *ListParticipantConversationParams) (chan ConversationsV1ParticipantConversation, chan error) {
	if params == nil {
	...
	response, err := c.PageParticipantConversationWithCtx(ctx, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
	} else {
		go c.streamParticipantConversation(ctx, response, params, recordChannel, errorChannel)
	}

	return recordChannel, errorChannel
}

func (c *ApiService) streamParticipantConversation(ctx context.Context, response *ListParticipantConversationResponse, params *ListParticipantConversationParams, recordChannel chan ConversationsV1ParticipantConversation, errorChannel chan error) {
	curRecord := 1
	...
	for response != nil {
		...
		record, err := client.GetNextWithCtx(ctx, c.baseURL, response, c.getNextListParticipantConversationResponse)
		if err != nil {
			errorChannel <- err
			break
		} else if record == nil {
...
func (c *ApiService) getNextListParticipantConversationResponse(ctx context.Context, nextPageUrl string) (interface{}, error) {
	if nextPageUrl == "" {
		return nil, nil
	}
