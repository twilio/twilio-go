...
func (c *ApiService) AssetVersionPage(ServiceSid string, AssetSid string, params *ListAssetVersionParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Services/{ServiceSid}/Assets/{AssetSid}/Versions"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
	path = strings.Replace(path, "{"+"AssetSid"+"}", AssetSid, -1)
...
func (c *ApiService) AssetVersionStream(ServiceSid string, AssetSid string, params *ListAssetVersionParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AssetVersionPage(ServiceSid, AssetSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists AssetVersion records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AssetVersionList(ServiceSid string, AssetSid string, params *ListAssetVersionParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AssetVersionPage(ServiceSid, AssetSid, params, "", "", fmt.Sprint(limits.PageSize))
