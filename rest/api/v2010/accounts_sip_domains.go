...
func (c *ApiService) ListSipDomain(params *ListSipDomainParams) (*ListSipDomainResponse, error) {
	path := "/2010-04-01/Accounts/{AccountSid}/SIP/Domains.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
func (c *ApiService) AccountsSIPDomainsPage(params *ListSipDomainParams, pageToken string, pageNumber string) (*ListSipDomainResponse, error) {
	path := "/2010-04-01/Accounts/{AccountSid}/SIP/Domains.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
//Lists AccountsSIPDomains records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AccountsSIPDomainsList(params *ListSipDomainParams, limit int) ([]ListSipDomainResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSipDomain(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListSipDomainResponse, len(resp))
...
//Streams AccountsSIPDomains records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) AccountsSIPDomainsStream(params *ListSipDomainParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSipDomain(params)
	if err != nil {
		return nil, err
