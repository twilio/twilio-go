...
func (c *ApiService) RecordingAddOnResultPage(ReferenceSid string, params *ListRecordingAddOnResultParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/2010-04-01/Accounts/{AccountSid}/Recordings/{ReferenceSid}/AddOnResults.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
func (c *ApiService) RecordingAddOnResultStream(ReferenceSid string, params *ListRecordingAddOnResultParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.RecordingAddOnResultPage(ReferenceSid, params, "", "", fmt.Sprint(limits.PageSize))
...
func (c *ApiService) RecordingAddOnResultList(ReferenceSid string, params *ListRecordingAddOnResultParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.RecordingAddOnResultPage(ReferenceSid, params, "", "", fmt.Sprint(limits.PageSize))
