...
func (c *ApiService) AvailablePhoneNumberMobilePage(CountryCode string, params *ListAvailablePhoneNumberMobileParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/Mobile.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
func (c *ApiService) AvailablePhoneNumberMobileStream(CountryCode string, params *ListAvailablePhoneNumberMobileParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AvailablePhoneNumberMobilePage(CountryCode, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists AvailablePhoneNumberMobile records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AvailablePhoneNumberMobileList(CountryCode string, params *ListAvailablePhoneNumberMobileParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AvailablePhoneNumberMobilePage(CountryCode, params, "", "", fmt.Sprint(limits.PageSize))
