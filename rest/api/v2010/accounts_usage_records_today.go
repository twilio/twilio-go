...
func (c *ApiService) PageUsageRecordToday(params *ListUsageRecordTodayParams, pageToken, pageNumber string) (*ListUsageRecordTodayResponse, error) {
	return c.PageUsageRecordTodayWithContext(context.TODO(), params, pageToken, pageNumber)
}

// Retrieve a single page of UsageRecordToday records from the API. Request is executed immediately.
func (c *ApiService) PageUsageRecordTodayWithContext(ctx context.Context, params *ListUsageRecordTodayParams, pageToken, pageNumber string) (*ListUsageRecordTodayResponse, error) {
...
func (c *ApiService) ListUsageRecordToday(params *ListUsageRecordTodayParams) ([]ApiV2010UsageRecordToday, error) {
	return c.ListUsageRecordTodayWithContext(context.TODO(), params)
}

// Lists UsageRecordToday records from the API as a list. Unlike stream, this operation is eager and loads 'limit' records into memory before returning.
func (c *ApiService) ListUsageRecordTodayWithContext(ctx context.Context, params *ListUsageRecordTodayParams) ([]ApiV2010UsageRecordToday, error) {
	response, errors := c.StreamUsageRecordTodayWithContext(ctx, params)
...
func (c *ApiService) StreamUsageRecordToday(params *ListUsageRecordTodayParams) (chan ApiV2010UsageRecordToday, chan error) {
	return c.StreamUsageRecordTodayWithContext(context.TODO(), params)
}

// Streams UsageRecordToday records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) StreamUsageRecordTodayWithContext(ctx context.Context, params *ListUsageRecordTodayParams) (chan ApiV2010UsageRecordToday, chan error) {
	if params == nil {
	...
	response, err := c.PageUsageRecordTodayWithContext(ctx, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
	} else {
		go c.streamUsageRecordTodayWithContext(ctx, response, params, recordChannel, errorChannel)
	}

	return recordChannel, errorChannel
}

func (c *ApiService) streamUsageRecordTodayWithContext(ctx context.Context, response *ListUsageRecordTodayResponse, params *ListUsageRecordTodayParams, recordChannel chan ApiV2010UsageRecordToday, errorChannel chan error) {
	curRecord := 1
	...
	for response != nil {
		...
		record, err := client.GetNextWithContext(ctx, c.baseURL, response, c.getNextListUsageRecordTodayResponseWithContext)
		if err != nil {
			errorChannel <- err
			break
		} else if record == nil {
...
func (c *ApiService) getNextListUsageRecordTodayResponseWithContext(ctx context.Context, nextPageUrl string) (interface{}, error) {
	if nextPageUrl == "" {
		return nil, nil
	}
