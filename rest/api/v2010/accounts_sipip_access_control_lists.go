...
func (c *ApiService) AccountsSIPIpAccessControlListsPage(params *ListSipIpAccessControlListParams, pageToken string, pageNumber string) (*ListSipIpAccessControlListResponse, error) {
	path := "/2010-04-01/Accounts/{AccountSid}/SIP/IpAccessControlLists.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
//Lists AccountsSIPIpAccessControlLists records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AccountsSIPIpAccessControlListsList(params *ListSipIpAccessControlListParams, limit int) ([]ListSipIpAccessControlListResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSipIpAccessControlList(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListSipIpAccessControlListResponse, len(resp))
...
//Streams AccountsSIPIpAccessControlLists records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) AccountsSIPIpAccessControlListsStream(params *ListSipIpAccessControlListParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSipIpAccessControlList(params)
	if err != nil {
		return nil, err
