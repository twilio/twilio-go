...
func (c *ApiService) AccountsKeysPage(params *ListKeyParams, pageToken string, pageNumber string) (*ListKeyResponse, error) {
	path := "/2010-04-01/Accounts/{AccountSid}/Keys.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
//Lists AccountsKeys records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AccountsKeysList(params *ListKeyParams, limit int) ([]ListKeyResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListKey(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListKeyResponse, len(resp))
...
//Streams AccountsKeys records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) AccountsKeysStream(params *ListKeyParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListKey(params)
	if err != nil {
		return nil, err
