...
func (c *ApiService) IncomingPhoneNumberMobilePage(params *ListIncomingPhoneNumberMobileParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/2010-04-01/Accounts/{AccountSid}/IncomingPhoneNumbers/Mobile.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
func (c *ApiService) IncomingPhoneNumberMobileStream(params *ListIncomingPhoneNumberMobileParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.IncomingPhoneNumberMobilePage(params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists IncomingPhoneNumberMobile records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) IncomingPhoneNumberMobileList(params *ListIncomingPhoneNumberMobileParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.IncomingPhoneNumberMobilePage(params, "", "", fmt.Sprint(limits.PageSize))
