...
// PageUsageRecordLastMonthWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) PageUsageRecordLastMonthWithMetadata(params *ListUsageRecordLastMonthParams, pageToken, pageNumber string) (*metadata.ResourceMetadata[ListUsageRecordLastMonthResponse], error) {
	path := "/2010-04-01/Accounts/{AccountSid}/Usage/Records/LastMonth.json"
	...
	metadataWrapper := metadata.NewResourceMetadata[ListUsageRecordLastMonthResponse](
		*ps,             // The page object
...
// ListUsageRecordLastMonthWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) ListUsageRecordLastMonthWithMetadata(params *ListUsageRecordLastMonthParams) (*metadata.ResourceMetadata[[]ApiV2010UsageRecordLastMonth], error) {
	response, errors := c.StreamUsageRecordLastMonthWithMetadata(params)
	...
	metadataWrapper := metadata.NewResourceMetadata[[]ApiV2010UsageRecordLastMonth](
		records,
...
// StreamUsageRecordLastMonthWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) StreamUsageRecordLastMonthWithMetadata(params *ListUsageRecordLastMonthParams) (*metadata.ResourceMetadata[chan ApiV2010UsageRecordLastMonth], chan error) {
	if params == nil {
		params = &ListUsageRecordLastMonthParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))
	...
	response, err := c.PageUsageRecordLastMonthWithMetadata(params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
		...
		resource := response.GetResource()
		go c.streamUsageRecordLastMonth(&resource, params, recordChannel, errorChannel)
	}

	metadataWrapper := metadata.NewResourceMetadata[chan ApiV2010UsageRecordLastMonth](
