...
func (c *ApiService) SipIpAccessControlListMappingPage(DomainSid string, params *ListSipIpAccessControlListMappingParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/2010-04-01/Accounts/{AccountSid}/SIP/Domains/{DomainSid}/IpAccessControlListMappings.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
func (c *ApiService) SipIpAccessControlListMappingStream(DomainSid string, params *ListSipIpAccessControlListMappingParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SipIpAccessControlListMappingPage(DomainSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists SipIpAccessControlListMapping records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) SipIpAccessControlListMappingList(DomainSid string, params *ListSipIpAccessControlListMappingParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.SipIpAccessControlListMappingPage(DomainSid, params, "", "", fmt.Sprint(limits.PageSize))
