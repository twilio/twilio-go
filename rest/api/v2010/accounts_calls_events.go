...
// PageCallEventWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) PageCallEventWithMetadata(CallSid string, params *ListCallEventParams, pageToken, pageNumber string) (*metadata.ResourceMetadata[ListCallEventResponse], error) {
	path := "/2010-04-01/Accounts/{AccountSid}/Calls/{CallSid}/Events.json"
	...
	metadataWrapper := metadata.NewResourceMetadata[ListCallEventResponse](
		*ps,             // The page object
...
// ListCallEventWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) ListCallEventWithMetadata(CallSid string, params *ListCallEventParams) (*metadata.ResourceMetadata[[]ApiV2010CallEvent], error) {
	response, errors := c.StreamCallEventWithMetadata(CallSid, params)
	...
	metadataWrapper := metadata.NewResourceMetadata[[]ApiV2010CallEvent](
		records,
...
// StreamCallEventWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) StreamCallEventWithMetadata(CallSid string, params *ListCallEventParams) (*metadata.ResourceMetadata[chan ApiV2010CallEvent], chan error) {
	if params == nil {
		params = &ListCallEventParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))
	...
	response, err := c.PageCallEventWithMetadata(CallSid, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
		...
		resource := response.GetResource()
		go c.streamCallEvent(&resource, params, recordChannel, errorChannel)
	}

	metadataWrapper := metadata.NewResourceMetadata[chan ApiV2010CallEvent](
