...
func (c *ApiService) AccountsAvailablePhoneNumbersMachineToMachinePage(CountryCode string, params *ListAvailablePhoneNumberMachineToMachineParams, pageToken string, pageNumber string) (*ListAvailablePhoneNumberMachineToMachineResponse, error) {
	path := "/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/MachineToMachine.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
//Lists AccountsAvailablePhoneNumbersMachineToMachine records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AccountsAvailablePhoneNumbersMachineToMachineList(CountryCode string, params *ListAvailablePhoneNumberMachineToMachineParams, limit int) ([]ListAvailablePhoneNumberMachineToMachineResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAvailablePhoneNumberMachineToMachine(CountryCode, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListAvailablePhoneNumberMachineToMachineResponse, len(resp))
...
//Streams AccountsAvailablePhoneNumbersMachineToMachine records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) AccountsAvailablePhoneNumbersMachineToMachineStream(CountryCode string, params *ListAvailablePhoneNumberMachineToMachineParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAvailablePhoneNumberMachineToMachine(CountryCode, params)
	if err != nil {
		return nil, err
