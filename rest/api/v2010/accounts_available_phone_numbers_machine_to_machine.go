...
func (c *ApiService) AvailablePhoneNumberMachineToMachinePage(CountryCode string, params *ListAvailablePhoneNumberMachineToMachineParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/MachineToMachine.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
func (c *ApiService) AvailablePhoneNumberMachineToMachineStream(CountryCode string, params *ListAvailablePhoneNumberMachineToMachineParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AvailablePhoneNumberMachineToMachinePage(CountryCode, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists AvailablePhoneNumberMachineToMachine records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AvailablePhoneNumberMachineToMachineList(CountryCode string, params *ListAvailablePhoneNumberMachineToMachineParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AvailablePhoneNumberMachineToMachinePage(CountryCode, params, "", "", fmt.Sprint(limits.PageSize))
