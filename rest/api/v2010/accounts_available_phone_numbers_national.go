...
func (c *ApiService) AccountsAvailablePhoneNumbersNationalPage(CountryCode string, params *ListAvailablePhoneNumberNationalParams, pageToken string, pageNumber string) (*ListAvailablePhoneNumberNationalResponse, error) {
	path := "/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/National.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
//Lists AccountsAvailablePhoneNumbersNational records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AccountsAvailablePhoneNumbersNationalList(CountryCode string, params *ListAvailablePhoneNumberNationalParams, limit int) ([]ListAvailablePhoneNumberNationalResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAvailablePhoneNumberNational(CountryCode, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListAvailablePhoneNumberNationalResponse, len(resp))
...
//Streams AccountsAvailablePhoneNumbersNational records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) AccountsAvailablePhoneNumbersNationalStream(CountryCode string, params *ListAvailablePhoneNumberNationalParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAvailablePhoneNumberNational(CountryCode, params)
	if err != nil {
		return nil, err
