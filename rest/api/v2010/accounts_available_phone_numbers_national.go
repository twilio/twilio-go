...
func (c *ApiService) AvailablePhoneNumberNationalPage(CountryCode string, params *ListAvailablePhoneNumberNationalParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/National.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
func (c *ApiService) AvailablePhoneNumberNationalStream(CountryCode string, params *ListAvailablePhoneNumberNationalParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AvailablePhoneNumberNationalPage(CountryCode, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists AvailablePhoneNumberNational records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AvailablePhoneNumberNationalList(CountryCode string, params *ListAvailablePhoneNumberNationalParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.AvailablePhoneNumberNationalPage(CountryCode, params, "", "", fmt.Sprint(limits.PageSize))
