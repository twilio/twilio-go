...
func (c *ApiService) PageAvailablePhoneNumberNational(CountryCode string, params *ListAvailablePhoneNumberNationalParams, pageToken, pageNumber string) (*ListAvailablePhoneNumberNationalResponse, error) {
	return c.PageAvailablePhoneNumberNationalWithContext(context.TODO(), CountryCode, params, pageToken, pageNumber)
}

// Retrieve a single page of AvailablePhoneNumberNational records from the API. Request is executed immediately.
func (c *ApiService) PageAvailablePhoneNumberNationalWithContext(ctx context.Context, CountryCode string, params *ListAvailablePhoneNumberNationalParams, pageToken, pageNumber string) (*ListAvailablePhoneNumberNationalResponse, error) {
...
// Lists AvailablePhoneNumberNational records from the API as a list. Unlike stream, this operation is eager and loads 'limit' records into memory before returning.
func (c *ApiService) ListAvailablePhoneNumberNationalWithContext(ctx context.Context, CountryCode string, params *ListAvailablePhoneNumberNationalParams) ([]ApiV2010AvailablePhoneNumberNational, error) {
	response, errors := c.StreamAvailablePhoneNumberNationalWithContext(ctx, CountryCode, params)
...
func (c *ApiService) StreamAvailablePhoneNumberNational(CountryCode string, params *ListAvailablePhoneNumberNationalParams) (chan ApiV2010AvailablePhoneNumberNational, chan error) {
	return c.StreamAvailablePhoneNumberNationalWithContext(context.TODO(), CountryCode, params)
}

// Streams AvailablePhoneNumberNational records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) StreamAvailablePhoneNumberNationalWithContext(ctx context.Context, CountryCode string, params *ListAvailablePhoneNumberNationalParams) (chan ApiV2010AvailablePhoneNumberNational, chan error) {
	if params == nil {
	...
	response, err := c.PageAvailablePhoneNumberNationalWithContext(ctx, CountryCode, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
	} else {
		go c.streamAvailablePhoneNumberNationalWithContext(ctx, response, params, recordChannel, errorChannel)
	}

	return recordChannel, errorChannel
}

func (c *ApiService) streamAvailablePhoneNumberNationalWithContext(ctx context.Context, response *ListAvailablePhoneNumberNationalResponse, params *ListAvailablePhoneNumberNationalParams, recordChannel chan ApiV2010AvailablePhoneNumberNational, errorChannel chan error) {
	curRecord := 1
	...
	for response != nil {
		...
		record, err := client.GetNextWithContext(ctx, c.baseURL, response, c.getNextListAvailablePhoneNumberNationalResponseWithContext)
		if err != nil {
			errorChannel <- err
			break
		} else if record == nil {
...
func (c *ApiService) getNextListAvailablePhoneNumberNationalResponseWithContext(ctx context.Context, nextPageUrl string) (interface{}, error) {
	if nextPageUrl == "" {
		return nil, nil
	}
