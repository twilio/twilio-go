...
func (c *ApiService) AccountsPage(params *ListAccountParams, pageToken string, pageNumber string) (*ListAccountResponse, error) {
	path := "/2010-04-01/Accounts.json"
...
//Lists Accounts records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AccountsList(params *ListAccountParams, limit int) ([]ListAccountResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAccount(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListAccountResponse, len(resp))
...
//Streams Accounts records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) AccountsStream(params *ListAccountParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAccount(params)
	if err != nil {
		return nil, err
