...
func (c *ApiService) AccountsAvailablePhoneNumbersLocalPage(CountryCode string, params *ListAvailablePhoneNumberLocalParams, pageToken string, pageNumber string) (*ListAvailablePhoneNumberLocalResponse, error) {
	path := "/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/Local.json"
	if params != nil && params.PathAccountSid != nil {
		path = strings.Replace(path, "{"+"AccountSid"+"}", *params.PathAccountSid, -1)
	} else {
...
//Lists AccountsAvailablePhoneNumbersLocal records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) AccountsAvailablePhoneNumbersLocalList(CountryCode string, params *ListAvailablePhoneNumberLocalParams, limit int) ([]ListAvailablePhoneNumberLocalResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAvailablePhoneNumberLocal(CountryCode, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListAvailablePhoneNumberLocalResponse, len(resp))
...
//Streams AccountsAvailablePhoneNumbersLocal records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) AccountsAvailablePhoneNumbersLocalStream(CountryCode string, params *ListAvailablePhoneNumberLocalParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListAvailablePhoneNumberLocal(CountryCode, params)
	if err != nil {
		return nil, err
