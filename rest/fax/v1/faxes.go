...
func (c *ApiService) ListFax(params *ListFaxParams) (*ListFaxResponse, error) {
	path := "/v1/Faxes"
...
func (c *ApiService) FaxesPage(params *ListFaxParams, pageToken string, pageNumber string) (*ListFaxResponse, error) {
	path := "/v1/Faxes"
...
//Lists Faxes records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) FaxesList(params *ListFaxParams, limit int) ([]ListFaxResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListFax(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListFaxResponse, len(resp))
...
//Streams Faxes records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) FaxesStream(params *ListFaxParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListFax(params)
	if err != nil {
		return nil, err
