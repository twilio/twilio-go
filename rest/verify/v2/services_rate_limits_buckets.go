...
func (c *ApiService) BucketPage(ServiceSid string, RateLimitSid string, params *ListBucketParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v2/Services/{ServiceSid}/RateLimits/{RateLimitSid}/Buckets"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
	path = strings.Replace(path, "{"+"RateLimitSid"+"}", RateLimitSid, -1)
...
func (c *ApiService) BucketStream(ServiceSid string, RateLimitSid string, params *ListBucketParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.BucketPage(ServiceSid, RateLimitSid, params, "", "", fmt.Sprint(limits.PageSize))
...
func (c *ApiService) BucketList(ServiceSid string, RateLimitSid string, params *ListBucketParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.BucketPage(ServiceSid, RateLimitSid, params, "", "", fmt.Sprint(limits.PageSize))
