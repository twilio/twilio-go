...
func (c *ApiService) ServicesMessagingConfigurationsPage(ServiceSid string, params *ListMessagingConfigurationParams, pageToken string, pageNumber string) (*ListMessagingConfigurationResponse, error) {
	path := "/v2/Services/{ServiceSid}/MessagingConfigurations"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
...
//Lists ServicesMessagingConfigurations records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ServicesMessagingConfigurationsList(ServiceSid string, params *ListMessagingConfigurationParams, limit int) ([]ListMessagingConfigurationResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListMessagingConfiguration(ServiceSid, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListMessagingConfigurationResponse, len(resp))
...
//Streams ServicesMessagingConfigurations records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) ServicesMessagingConfigurationsStream(ServiceSid string, params *ListMessagingConfigurationParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListMessagingConfiguration(ServiceSid, params)
	if err != nil {
		return nil, err
