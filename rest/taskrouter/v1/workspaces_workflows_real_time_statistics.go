...
// FetchWorkflowRealTimeStatisticsWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) FetchWorkflowRealTimeStatisticsWithMetadata(WorkspaceSid string, WorkflowSid string, params *FetchWorkflowRealTimeStatisticsParams) (*metadata.ResourceMetadata[TaskrouterV1WorkflowRealTimeStatistics], error) {
	path := "/v1/Workspaces/{WorkspaceSid}/Workflows/{WorkflowSid}/RealTimeStatistics"
	path = strings.Replace(path, "{"+"WorkspaceSid"+"}", WorkspaceSid, -1)
	path = strings.Replace(path, "{"+"WorkflowSid"+"}", WorkflowSid, -1)
	...
	metadataWrapper := metadata.NewResourceMetadata[TaskrouterV1WorkflowRealTimeStatistics](
