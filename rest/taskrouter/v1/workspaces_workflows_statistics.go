...
// FetchWorkflowStatisticsWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) FetchWorkflowStatisticsWithMetadata(WorkspaceSid string, WorkflowSid string, params *FetchWorkflowStatisticsParams) (*metadata.ResourceMetadata[TaskrouterV1WorkflowStatistics], error) {
	path := "/v1/Workspaces/{WorkspaceSid}/Workflows/{WorkflowSid}/Statistics"
	path = strings.Replace(path, "{"+"WorkspaceSid"+"}", WorkspaceSid, -1)
	path = strings.Replace(path, "{"+"WorkflowSid"+"}", WorkflowSid, -1)
	...
	metadataWrapper := metadata.NewResourceMetadata[TaskrouterV1WorkflowStatistics](
