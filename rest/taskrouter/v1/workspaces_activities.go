...
func (c *ApiService) ActivityPage(WorkspaceSid string, params *ListActivityParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Workspaces/{WorkspaceSid}/Activities"
	path = strings.Replace(path, "{"+"WorkspaceSid"+"}", WorkspaceSid, -1)
...
func (c *ApiService) ActivityStream(WorkspaceSid string, params *ListActivityParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ActivityPage(WorkspaceSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists Activity records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ActivityList(WorkspaceSid string, params *ListActivityParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ActivityPage(WorkspaceSid, params, "", "", fmt.Sprint(limits.PageSize))
