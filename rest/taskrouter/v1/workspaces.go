...
func (c *ApiService) WorkspacesPage(params *ListWorkspaceParams, pageToken string, pageNumber string) (*ListWorkspaceResponse, error) {
	path := "/v1/Workspaces"
...
//Lists Workspaces records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) WorkspacesList(params *ListWorkspaceParams, limit int) ([]ListWorkspaceResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListWorkspace(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListWorkspaceResponse, len(resp))
...
//Streams Workspaces records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) WorkspacesStream(params *ListWorkspaceParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListWorkspace(params)
	if err != nil {
		return nil, err
