...
func (c *ApiService) WorkspacePage(params *ListWorkspaceParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v1/Workspaces"
...
func (c *ApiService) WorkspaceStream(params *ListWorkspaceParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.WorkspacePage(params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists Workspace records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) WorkspaceList(params *ListWorkspaceParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.WorkspacePage(params, "", "", fmt.Sprint(limits.PageSize))
