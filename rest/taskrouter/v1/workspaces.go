...
func (c *ApiService) ListWorkspace(params *ListWorkspaceParams, limit *int) ([]*ListWorkspaceResponse, error) {
	...
	curRecord := 0
	var records []*ListWorkspaceResponse

	for response != nil {
		records = append(records, response)

		var record interface{}
		if record, err = client.GetNext(response, &curRecord, limit, c.getNextListWorkspaceResponse); record == nil || err != nil {
			return records, err
		}
...
func (c *ApiService) StreamWorkspace(params *ListWorkspaceParams, limit *int) (chan *ListWorkspaceResponse, error) {
	params.SetPageSize(client.ReadLimits(params.PageSize, limit))
	...
	channel := make(chan *ListWorkspaceResponse, 1)
...
func (c *ApiService) getNextListWorkspaceResponse(nextPageUri string) (interface{}, error) {
	if nextPageUri == "" {
		return nil, nil
	}
