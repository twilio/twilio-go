# TrusthubV1TrustProductProvisionalCopy

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**TrustProductSid** | Pointer to **string** | The unique SID identifier of the Trust Product. |
**AccountSid** | Pointer to **string** | The unique SID identifier of the Account. |
**PolicySid** | Pointer to **string** | The unique SID identifier of the Policy. |
**FriendlyName** | Pointer to **string** | A human readable description of this resource, up to 64 characters. |
**Status** | Pointer to **string** | The verification status of the Customer-Profile resource. |
**Email** | Pointer to **string** | The email address that will receive updates when the trust product resource changes status. |
**StatusCallback** | Pointer to **string** | The URL we call to inform your application of status changes. |
**ValidUntil** | Pointer to [**time.Time**](time.Time.md) | The date and time in GMT in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format when the resource will be valid until. |
**DateCreated** | Pointer to [**time.Time**](time.Time.md) | The date that this Trust Products Provisional Copy was created, given in ISO 8601 format. |
**DateUpdated** | Pointer to [**time.Time**](time.Time.md) | The date that this Trust Products Provisional Copy was updated, given in ISO 8601 format. |
**Url** | Pointer to **string** | The URL of this resource. |

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


