...
func (c *ApiService) ServicesPage(params *ListServiceParams, pageToken string, pageNumber string) (*ListServiceResponse, error) {
	path := "/v1/Services"
...
//Lists Services records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ServicesList(params *ListServiceParams, limit int) ([]ListServiceResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListService(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListServiceResponse, len(resp))
...
//Streams Services records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) ServicesStream(params *ListServiceParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListService(params)
	if err != nil {
		return nil, err
