...
func (c *ApiService) SubscriptionsSubscribedEventsPage(SubscriptionSid string, params *ListSubscribedEventParams, pageToken string, pageNumber string) (*ListSubscribedEventResponse, error) {
	path := "/v1/Subscriptions/{SubscriptionSid}/SubscribedEvents"
	path = strings.Replace(path, "{"+"SubscriptionSid"+"}", SubscriptionSid, -1)
...
//Lists SubscriptionsSubscribedEvents records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) SubscriptionsSubscribedEventsList(SubscriptionSid string, params *ListSubscribedEventParams, limit int) ([]ListSubscribedEventResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSubscribedEvent(SubscriptionSid, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListSubscribedEventResponse, len(resp))
...
//Streams SubscriptionsSubscribedEvents records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) SubscriptionsSubscribedEventsStream(SubscriptionSid string, params *ListSubscribedEventParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListSubscribedEvent(SubscriptionSid, params)
	if err != nil {
		return nil, err
