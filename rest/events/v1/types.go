...
// FetchEventTypeWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) FetchEventTypeWithMetadata(Type string) (*metadata.ResourceMetadata[EventsV1EventType], error) {
	path := "/v1/Types/{Type}"
	path = strings.Replace(path, "{"+"Type"+"}", Type, -1)
	...
	metadataWrapper := metadata.NewResourceMetadata[EventsV1EventType](
...
// PageEventTypeWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) PageEventTypeWithMetadata(params *ListEventTypeParams, pageToken, pageNumber string) (*metadata.ResourceMetadata[ListEventTypeResponse], error) {
	path := "/v1/Types"
	...
	metadataWrapper := metadata.NewResourceMetadata[ListEventTypeResponse](
		*ps,             // The page object
		resp.StatusCode, // HTTP status code
		resp.Header,     // HTTP headers
	)
...
// ListEventTypeWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) ListEventTypeWithMetadata(params *ListEventTypeParams) (*metadata.ResourceMetadata[[]EventsV1EventType], error) {
	response, errors := c.StreamEventTypeWithMetadata(params)
...
// StreamEventTypeWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) StreamEventTypeWithMetadata(params *ListEventTypeParams) (*metadata.ResourceMetadata[chan EventsV1EventType], chan error) {
	if params == nil {
		params = &ListEventTypeParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))
	...
	response, err := c.PageEventTypeWithMetadata(params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
		...
		resource := response.GetResource()
		go c.streamEventType(&resource, params, recordChannel, errorChannel)
	}

	metadataWrapper := metadata.NewResourceMetadata[chan EventsV1EventType](
