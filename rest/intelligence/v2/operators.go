...
func (c *ApiService) FetchOperatorWithMetadata(Sid string) (*metadata.ResourceMetadata[IntelligenceV2Operator], error) {
	path := "/v2/Operators/{Sid}"
	path = strings.Replace(path, "{"+"Sid"+"}", Sid, -1)
	...
	metadataWrapper := metadata.NewResourceMetadata[IntelligenceV2Operator](
...
// PageOperatorWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) PageOperatorWithMetadata(params *ListOperatorParams, pageToken, pageNumber string) (*metadata.ResourceMetadata[ListOperatorResponse], error) {
	path := "/v2/Operators"
	...
	metadataWrapper := metadata.NewResourceMetadata[ListOperatorResponse](
		*ps,             // The page object
		resp.StatusCode, // HTTP status code
		resp.Header,     // HTTP headers
	)
...
// ListOperatorWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) ListOperatorWithMetadata(params *ListOperatorParams) (*metadata.ResourceMetadata[[]IntelligenceV2Operator], error) {
	response, errors := c.StreamOperatorWithMetadata(params)
...
// StreamOperatorWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) StreamOperatorWithMetadata(params *ListOperatorParams) (*metadata.ResourceMetadata[chan IntelligenceV2Operator], chan error) {
	if params == nil {
		params = &ListOperatorParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))
	...
	response, err := c.PageOperatorWithMetadata(params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
		...
		resource := response.GetResource()
		go c.streamOperator(&resource, params, recordChannel, errorChannel)
	}

	metadataWrapper := metadata.NewResourceMetadata[chan IntelligenceV2Operator](
