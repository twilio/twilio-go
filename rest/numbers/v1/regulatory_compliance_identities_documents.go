...
func (c *ApiService) PageDocument(IdentitySid string, params *ListDocumentParams, pageToken, pageNumber string) (*ListDocumentResponse, error) {
	path := "/v1/RegulatoryCompliance/Identities/{IdentitySid}/Documents"

	path = strings.Replace(path, "{"+"IdentitySid"+"}", IdentitySid, -1)
...
func (c *ApiService) ListDocument(IdentitySid string, params *ListDocumentParams) ([]NumbersV1Document, error) {
	response, errors := c.StreamDocument(IdentitySid, params)

	records := make([]NumbersV1Document, 0)
...
func (c *ApiService) StreamDocument(IdentitySid string, params *ListDocumentParams) (chan NumbersV1Document, chan error) {
	if params == nil {
		params = &ListDocumentParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))

	recordChannel := make(chan NumbersV1Document, 1)
	errorChannel := make(chan error, 1)

	response, err := c.PageDocument(IdentitySid, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
...
func (c *ApiService) streamDocument(response *ListDocumentResponse, params *ListDocumentParams, recordChannel chan NumbersV1Document, errorChannel chan error) {
	curRecord := 1
