...
// PageDependentHostedNumberOrderWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) PageDependentHostedNumberOrderWithMetadata(SigningDocumentSid string, params *ListDependentHostedNumberOrderParams, pageToken, pageNumber string) (*metadata.ResourceMetadata[ListDependentHostedNumberOrderResponse], error) {
	path := "/v2/HostedNumber/AuthorizationDocuments/{SigningDocumentSid}/DependentHostedNumberOrders"
	...
	metadataWrapper := metadata.NewResourceMetadata[ListDependentHostedNumberOrderResponse](
		*ps,             // The page object
...
// ListDependentHostedNumberOrderWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) ListDependentHostedNumberOrderWithMetadata(SigningDocumentSid string, params *ListDependentHostedNumberOrderParams) (*metadata.ResourceMetadata[[]NumbersV2DependentHostedNumberOrder], error) {
	response, errors := c.StreamDependentHostedNumberOrderWithMetadata(SigningDocumentSid, params)
	...
	metadataWrapper := metadata.NewResourceMetadata[[]NumbersV2DependentHostedNumberOrder](
		records,
...
// StreamDependentHostedNumberOrderWithMetadata returns response with metadata like status code and response headers
func (c *ApiService) StreamDependentHostedNumberOrderWithMetadata(SigningDocumentSid string, params *ListDependentHostedNumberOrderParams) (*metadata.ResourceMetadata[chan NumbersV2DependentHostedNumberOrder], chan error) {
	if params == nil {
		params = &ListDependentHostedNumberOrderParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))
	...
	response, err := c.PageDependentHostedNumberOrderWithMetadata(SigningDocumentSid, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
		...
		resource := response.GetResource()
		go c.streamDependentHostedNumberOrder(&resource, params, recordChannel, errorChannel)
	}

	metadataWrapper := metadata.NewResourceMetadata[chan NumbersV2DependentHostedNumberOrder](
