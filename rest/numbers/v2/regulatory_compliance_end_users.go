...
func (c *ApiService) RegulatoryComplianceEndUsersPage(params *ListEndUserParams, pageToken string, pageNumber string) (*ListEndUserResponse, error) {
	path := "/v2/RegulatoryCompliance/EndUsers"
...
//Lists RegulatoryComplianceEndUsers records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) RegulatoryComplianceEndUsersList(params *ListEndUserParams, limit int) ([]ListEndUserResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListEndUser(params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListEndUserResponse, len(resp))
...
//Streams RegulatoryComplianceEndUsers records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) RegulatoryComplianceEndUsersStream(params *ListEndUserParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListEndUser(params)
	if err != nil {
		return nil, err
