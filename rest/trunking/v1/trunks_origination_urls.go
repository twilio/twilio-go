...
func (c *ApiService) TrunksOriginationUrlsPage(TrunkSid string, params *ListOriginationUrlParams, pageToken string, pageNumber string) (*ListOriginationUrlResponse, error) {
	path := "/v1/Trunks/{TrunkSid}/OriginationUrls"
	path = strings.Replace(path, "{"+"TrunkSid"+"}", TrunkSid, -1)
...
//Lists TrunksOriginationUrls records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) TrunksOriginationUrlsList(TrunkSid string, params *ListOriginationUrlParams, limit int) ([]ListOriginationUrlResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListOriginationUrl(TrunkSid, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListOriginationUrlResponse, len(resp))
...
//Streams TrunksOriginationUrls records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) TrunksOriginationUrlsStream(TrunkSid string, params *ListOriginationUrlParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListOriginationUrl(TrunkSid, params)
	if err != nil {
		return nil, err
