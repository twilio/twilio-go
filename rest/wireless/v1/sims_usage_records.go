...
func (c *ApiService) PageUsageRecordWithMetadata(SimSid string, params *ListUsageRecordParams, pageToken, pageNumber string) (*metadata.ResourceMetadata[ListUsageRecordResponse], error) {
	path := "/v1/Sims/{SimSid}/UsageRecords"
...
func (c *ApiService) ListUsageRecordWithMetadata(SimSid string, params *ListUsageRecordParams) (*metadata.ResourceMetadata[[]WirelessV1UsageRecord], error) {
	response, errors := c.StreamUsageRecordWithMetadata(SimSid, params)
	...
	metadataWrapper := metadata.NewResourceMetadata[[]WirelessV1UsageRecord](
		records,
...
func (c *ApiService) StreamUsageRecordWithMetadata(SimSid string, params *ListUsageRecordParams) (*metadata.ResourceMetadata[chan WirelessV1UsageRecord], chan error) {
	if params == nil {
		params = &ListUsageRecordParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))
	...
	response, err := c.PageUsageRecordWithMetadata(SimSid, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
	...
	metadataWrapper := metadata.NewResourceMetadata[chan WirelessV1UsageRecord](
