...
func (c *ApiService) SimsUsageRecordsPage(SimSid string, params *ListUsageRecordParams, pageToken string, pageNumber string) (*ListUsageRecordResponse, error) {
	path := "/v1/Sims/{SimSid}/UsageRecords"
	path = strings.Replace(path, "{"+"SimSid"+"}", SimSid, -1)
...
//Lists SimsUsageRecords records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) SimsUsageRecordsList(SimSid string, params *ListUsageRecordParams, limit int) ([]ListUsageRecordResponse, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListUsageRecord(SimSid, params)
	if err != nil {
		return nil, err
	}
	...
	ret := make([]ListUsageRecordResponse, len(resp))
...
//Streams SimsUsageRecords records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) SimsUsageRecordsStream(SimSid string, params *ListUsageRecordParams, limit int) (chan interface{}, error) {
	params.SetPageSize(c.requestHandler.ReadLimits(params.PageSize, limit))
	response, err := c.ListUsageRecord(SimSid, params)
	if err != nil {
		return nil, err
