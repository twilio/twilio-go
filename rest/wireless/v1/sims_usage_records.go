...
func (c *ApiService) PageUsageRecord(SimSid string, params *ListUsageRecordParams, pageToken, pageNumber string) (*ListUsageRecordResponse, error) {
	return c.PageUsageRecordWithCtx(context.TODO(), SimSid, params, pageToken, pageNumber)
}

// Retrieve a single page of UsageRecord records from the API. Request is executed immediately.
func (c *ApiService) PageUsageRecordWithCtx(ctx context.Context, SimSid string, params *ListUsageRecordParams, pageToken, pageNumber string) (*ListUsageRecordResponse, error) {
...
func (c *ApiService) ListUsageRecord(SimSid string, params *ListUsageRecordParams) ([]WirelessV1UsageRecord, error) {
	return c.ListUsageRecordWithCtx(context.TODO(), SimSid, params)
}

// Lists UsageRecord records from the API as a list. Unlike stream, this operation is eager and loads 'limit' records into memory before returning.
func (c *ApiService) ListUsageRecordWithCtx(ctx context.Context, SimSid string, params *ListUsageRecordParams) ([]WirelessV1UsageRecord, error) {
	response, errors := c.StreamUsageRecordWithCtx(ctx, SimSid, params)
...
func (c *ApiService) StreamUsageRecord(SimSid string, params *ListUsageRecordParams) (chan WirelessV1UsageRecord, chan error) {
	return c.StreamUsageRecordWithCtx(context.TODO(), SimSid, params)
}

// Streams UsageRecord records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) StreamUsageRecordWithCtx(ctx context.Context, SimSid string, params *ListUsageRecordParams) (chan WirelessV1UsageRecord, chan error) {
	if params == nil {
	...
	response, err := c.PageUsageRecordWithCtx(ctx, SimSid, params, "", "")
	if err != nil {
		errorChannel <- err
		close(recordChannel)
		close(errorChannel)
...
func (c *ApiService) streamUsageRecord(ctx context.Context, response *ListUsageRecordResponse, params *ListUsageRecordParams, recordChannel chan WirelessV1UsageRecord, errorChannel chan error) {
	curRecord := 1
