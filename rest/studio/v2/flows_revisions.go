...
func (c *ApiService) FlowRevisionPage(Sid string, params *ListFlowRevisionParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v2/Flows/{Sid}/Revisions"
	path = strings.Replace(path, "{"+"Sid"+"}", Sid, -1)
...
func (c *ApiService) FlowRevisionStream(Sid string, params *ListFlowRevisionParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.FlowRevisionPage(Sid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists FlowRevision records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) FlowRevisionList(Sid string, params *ListFlowRevisionParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.FlowRevisionPage(Sid, params, "", "", fmt.Sprint(limits.PageSize))
