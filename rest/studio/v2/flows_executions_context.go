...
func (c *ApiService) FetchExecutionContextWithMetadata(FlowSid string, ExecutionSid string) (*metadata.ResourceMetadata[StudioV2ExecutionContext], error) {
	path := "/v2/Flows/{FlowSid}/Executions/{ExecutionSid}/Context"
	path = strings.Replace(path, "{"+"FlowSid"+"}", FlowSid, -1)
	path = strings.Replace(path, "{"+"ExecutionSid"+"}", ExecutionSid, -1)
	...
	metadataWrapper := metadata.NewResourceMetadata[StudioV2ExecutionContext](
