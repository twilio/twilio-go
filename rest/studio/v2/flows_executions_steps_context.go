...
func (c *ApiService) FetchExecutionStepContextWithMetadata(FlowSid string, ExecutionSid string, StepSid string) (*metadata.ResourceMetadata[StudioV2ExecutionStepContext], error) {
	path := "/v2/Flows/{FlowSid}/Executions/{ExecutionSid}/Steps/{StepSid}/Context"
	path = strings.Replace(path, "{"+"FlowSid"+"}", FlowSid, -1)
	path = strings.Replace(path, "{"+"ExecutionSid"+"}", ExecutionSid, -1)
	path = strings.Replace(path, "{"+"StepSid"+"}", StepSid, -1)
	...
	metadataWrapper := metadata.NewResourceMetadata[StudioV2ExecutionStepContext](
