...
func (c *ApiService) ChannelWebhookPage(ServiceSid string, ChannelSid string, params *ListChannelWebhookParams, pageToken string, pageNumber string, pageSize string) *client.Page {
	path := "/v2/Services/{ServiceSid}/Channels/{ChannelSid}/Webhooks"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
	path = strings.Replace(path, "{"+"ChannelSid"+"}", ChannelSid, -1)
...
func (c *ApiService) ChannelWebhookStream(ServiceSid string, ChannelSid string, params *ListChannelWebhookParams, meta client.PaginationData) chan map[string]interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ChannelWebhookPage(ServiceSid, ChannelSid, params, "", "", fmt.Sprint(limits.PageSize))
	return c.requestHandler.Stream(page, limits.Limit, limits.PageLimit)
}

//Lists ChannelWebhook records from the API as a list. Unlike stream, this operation is eager and will loads 'limit' records into memory before returning.
func (c *ApiService) ChannelWebhookList(ServiceSid string, ChannelSid string, params *ListChannelWebhookParams, meta client.PaginationData) []interface{} {
	limits := c.requestHandler.ReadLimits(meta)
	page := c.ChannelWebhookPage(ServiceSid, ChannelSid, params, "", "", fmt.Sprint(limits.PageSize))
