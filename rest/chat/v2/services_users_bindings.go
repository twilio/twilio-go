...
func (c *ApiService) ListUserBinding(ServiceSid string, UserSid string, params *ListUserBindingParams) ([]ChatV2ServiceUserUserBinding, error) {
	if params == nil {
	...
	curRecord := 0
	var records []ChatV2ServiceUserUserBinding

	for response != nil {
		records = append(records, response.Bindings...)

		var record interface{}
		if record, err = client.GetNext(response, &curRecord, params.Limit, c.getNextListUserBindingResponse); record == nil || err != nil {
			return records, err
		}
...
func (c *ApiService) StreamUserBinding(ServiceSid string, UserSid string, params *ListUserBindingParams) (chan ChatV2ServiceUserUserBinding, error) {
	if params == nil {
		params = &ListUserBindingParams{}
	}
	params.SetPageSize(client.ReadLimits(params.PageSize, params.Limit))
	...
	channel := make(chan ChatV2ServiceUserUserBinding, 1)
