...
//
func (c *ApiService) FetchBindingWithCtx(ctx context.Context, ServiceSid string, Sid string) (*ChatV2Binding, error) {
	path := "/v2/Services/{ServiceSid}/Bindings/{Sid}"
	path = strings.Replace(path, "{"+"ServiceSid"+"}", ServiceSid, -1)
	path = strings.Replace(path, "{"+"Sid"+"}", Sid, -1)
...
// Lists Binding records from the API as a list. Unlike stream, this operation is eager and loads 'limit' records into memory before returning.
func (c *ApiService) ListBindingWithCtx(ctx context.Context, ServiceSid string, params *ListBindingParams) ([]ChatV2Binding, error) {
...
// Streams Binding records from the API as a channel stream. This operation lazily loads records as efficiently as possible until the limit is reached.
func (c *ApiService) StreamBindingWithCtx(ctx context.Context, ServiceSid string, params *ListBindingParams) (chan ChatV2Binding, chan error) {
	if params == nil {
...
func (c *ApiService) streamBinding(ctx context.Context, response *ListBindingResponse, params *ListBindingParams, recordChannel chan ChatV2Binding, errorChannel chan error) {
	curRecord := 1
